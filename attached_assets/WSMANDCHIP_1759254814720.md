...
class LatticeMemory:
    ...
    def _id(self, key: dict, payload: dict, features: list[float]) -> str:
        h = hashlib.sha256((str(key)+str(payload)+str(features)).encode()).hexdigest()
        return f"ns_{h[:12]}"
    ...
    def _struct(self, key_q: dict, key_v: dict) -> float:
        # structural similarity: shared fields + exact equality weighting
        if not key_q or not key_v:
            return 0.0
        fields = set(key_q.keys()) | set(key_v.keys())
        if not fields: return 0.0
        score = 0.0
        for f in fields:
            if f in key_q and f in key_v:
                score += 1.0 if key_q[f] == key_v[f] else 0.3
        return score/len(fields)

    def write(self, key: dict, payload: dict, features: list[float]) -> tuple[str,bool]:
        node_id = self._id(key,payload,features)
        for i, (nid, k, p, f) in enumerate(self.nodes):
            if nid == node_id:
                self.nodes[i] = (nid, key, payload, features)  # update
                return nid, True
        self.nodes.append((node_id, key, payload, features))
        return node_id, False

    def query(self, features: list[float], k: int = 5):
        # frontier by blended structural+numeric similarity (structure from last write's key)
        if not self.nodes:
            return []
        key_q = self.nodes[-1][1]  # cheap heuristic; swap to explicit param if desired
        scored: list[tuple[float,str]] = []
        for (nid, kdict, payload, f) in self.n
```
